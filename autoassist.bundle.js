...
async function voicechat({
  ...
  async function prompt(text, polite) {
    ...
    // 7. Monitor for both mic noise and WebRTC output if polite
    let stopMonitorMic = null;
    let stopMonitorWebRTC = null;

    if (polite) {
      if (polite === true || polite === 'mic') {
        stopMonitorMic = monitorMicNoise(context, micStream, async () => {
          debug && console.log('🤫 Mic noise detected during TTS, stopping early.');
          if (!synthStoppedManually) {
            synthStoppedManually = true;
            synthSource.stop(); // stop TTS early if mic noise detected
          }
        });
      }

      if (polite === true || polite === 'ai') {
        stopMonitorWebRTC = monitorWebRTCOutput(async () => {
          debug && console.log('🤫 WebRTC output detected, stopping TTS early.');
          if (!synthStoppedManually) {
            synthStoppedManually = true;
            synthSource.stop(); // stop TTS early if WebRTC output is active
          }
        });
      }
    }
    ...
    synthSource.onended = async () => {
      if (stopMonitorMic) stopMonitorMic();
      if (stopMonitorWebRTC) stopMonitorWebRTC();
    ...
    // Monitor mic noise
    function monitorMicNoise(audioContext, micStream, onNoiseDetected, threshold = 0.4) {
    ...
    // Monitor WebRTC output with noise threshold
    function monitorWebRTCOutput(onDetected, threshold = 0.1) {
      const checkWebRTCTrack = () => {
        ...
        if (track && track.readyState === 'live') {
          ...
          if (normalized > threshold) {
            onDetected(); // Trigger action when output exceeds threshold
          } else {
            setTimeout(checkWebRTCTrack, 100); // Re-check every 100ms if below threshold
          }
        } else {
          setTimeout(checkWebRTCTrack, 100); // Re-check every 100ms
        }
      };

      checkWebRTCTrack();

      // Return a stop function to clear the monitoring if needed
      return () => {
        clearTimeout(checkWebRTCTrack);
      };
    }
  }
